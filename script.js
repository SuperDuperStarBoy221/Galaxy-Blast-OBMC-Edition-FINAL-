...
function openSettings(origin){
    ...
    if(origin==='sp-pause'){
        ...
        showMpMobileControls(false); applyMpMobileLayout(false);
    } else { hideAll(); }
...
function closeSettings(){
    ...
    if(origin==='sp-pause'){
        ...
        showMpMobileControls(true); applyMpMobileLayout(isTouchDevice);
    } else { showStart(); }
}
...
function hideAll(){
    ...
    document.getElementById('mp-vs-settings-btn').classList.remove('show');
    document.getElementById('mp-vs-quit-btn').classList.remove('show');
    showMobPauseBtn(false); showMpMobileControls(false); applyMpMobileLayout(false);
}
...
function showMpResult(){
    ...
    document.getElementById('mp-vs-settings-btn').classList.remove('show');
    document.getElementById('mp-vs-quit-btn').classList.remove('show'); showMobPauseBtn(false); showMpMobileControls(false); applyMpMobileLayout(false);
...
function beginMp(m){
    ...
    applyMpMobileLayout(isTouchDevice);
    document.getElementById('mp-vs-settings-btn').classList.remove('show');
    document.getElementById('mp-vs-quit-btn').classList.remove('show');
...
/*full-bleed top/bottom split-screen layout for mobile multiplayer*/
function fitMpFields(){
    [1,2].forEach(function(n){
        var game=document.getElementById("mp-game-"+n);
        var field=game.parentElement;
        if(!field)return;
        var fw=field.clientWidth, fh=field.clientHeight;
        if(!fw||!fh)return;
        var s=Math.min(fw/280, fh/420);
        if(!isFinite(s)||s<=0)return;
        game.style.transform="translate(-50%,-50%) scale("+s+")";
    });
}
function applyMpMobileLayout(active){
    var wrap=document.getElementById("mp-wrapper"), root=document.getElementById("scale-root");
    if(active){
        document.body.classList.add("mp-mobile-active");
        wrap.classList.add("mp-wrapper-mobile");
        root.style.transform="none"; root.style.width="100vw";
        window.addEventListener("resize",fitMpFields);
        requestAnimationFrame(function(){requestAnimationFrame(fitMpFields);});
    } else {
        document.body.classList.remove("mp-mobile-active");
        wrap.classList.remove("mp-wrapper-mobile");
        root.style.width="";
        [1,2].forEach(function(n){document.getElementById("mp-game-"+n).style.transform="";});
        window.removeEventListener("resize",fitMpFields);
